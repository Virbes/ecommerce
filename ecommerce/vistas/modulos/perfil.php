	...
	<div class="container">
		...
		<ul class="nav nav-tabs">
	  		...
	  		<li class="active">	  			
			  	...
			  	<i class="fa fa-list-ul"></i> MIS PEDIDOS</a>
	  		</li>

	  		<li> 				
		  		<a data-toggle="tab" href="#deseos">
		  		<i class="fa fa-gift"></i> PRODUCTOS QUE ME INTERESAN</a>
	  		</li>
			...
			======================================-->
	  		...
	  		<div id="compras" class="tab-pane fade in active">
				...
				<?php
					...
					$pedidos = ControladorPedidos::getPedidos($valor);

					if(!$pedidos){
						echo '<div class="col-xs-12 text-center error404">
				    		<h1><small>ʕ•́ᴥ•̀ʔっ</small></h1>
				    		<h2>Aún no tienes pedidos realizados en esta tienda</h2>
				  		</div>';
					}else{
						...
						foreach ($pedidos as $key => $value1) {
							...
							$fec = $pedidos[$key]['fecha'];

							$produkts = (json_decode($pedidos[$key]['productos'], true));
							...
							for ($i = 0; $i < count($produkts); $i++) {
								...
								$detalles_producto = ModeloProductos::getProduct($produkts[$i]['idProducto']);
									...
									<div class="panel-body">
										<div class="col-md-2 col-sm-6 col-xs-12">
											<figure>
												<img class="img-thumbnail" src="http://localhost/admint/'.$detalles_producto['portada'].'">
											</figure>
										</div>

									<div class="col-sm-6 col-xs-12">
										<h1><small>'.$detalles_producto['titulo'].'</small></h1>
										<p>'.$detalles_producto['descripcion'].'</p>
										<p><b style="color: #777">Precio: </b>$'.$produkts[$i]['precio'].'</p>
										<p><b style="color: #777">Cantidad: </b>'.$produkts[$i]['cantidad'].'</p>
										<p><b style="color: #777">Código: </b>'.$detalles_producto['codigo'].'</p>					
									</div>
							...
							echo '
							<hr>
							<h4 style="color: #7b7b7b; margin-left: 30px;">TOTAL: $'.$pedidos[$key]['total'].' MXN</h4>
							<h4 style="text-align: right; margin-right: 20px;"><small>Pedido el '.substr($fec,0,-8).'</small></h4>
							</div><br><hr><br>';
			...
			<?php
				...
				if(!$deseos){
					...
					echo '<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center error404">
			    		...
			    		<h2>Aún no tiene productos en su lista</h2>
										...
										<div class="btn-group pull-right">
											...
											<button type="button" class="btn btn-danger btn-xs quitarDeseo" idDeseo="'.$value1["id"].'" data-toggle="tooltip" title="Quitar de mi lista">
												...
												if($value2["oferta"] != 0){
													...
													echo '<button type="button" class="btn btn-default btn-xs agregarCarrito"  idProducto="'.$value2["id"].'" imagen="'.$servidor.$value2["portada"].'" titulo="'.$value2["titulo"].'" precio="'.$value2["precioOferta"].'" tipo="'.$value2["tipo"].'" peso="'.$value2["peso"].'" data-toggle="tooltip" title="Agregar al carrito">
													...
													echo '<button type="button" class="btn btn-default btn-xs agregarCarrito"  idProducto="'.$value2["id"].'" imagen="'.$servidor.$value2["portada"].'" titulo="'.$value2["titulo"].'" precio="'.$value2["precio"].'" peso="'.$value2["peso"].'" data-toggle="tooltip" title="Agregar al carrito">
					...
					<form method="post" enctype="multipart/form-data">
						...
						<div class="col-md-3 col-sm-4 col-xs-12 text-center">
							...
							if($_SESSION["foto"] != ""){
								...
								echo '<img src="http://localhost/admint/ecommerce/'.$_SESSION["foto"].'" class="img-thumbnail">';
