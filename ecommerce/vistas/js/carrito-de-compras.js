$('#realizar_pedido').click(function(e) {
	...
	var sumaCarrito = localStorage.getItem("sumaCesta");

	var nombre_reibidor = $('#nuevoRecibidor').val();
	var telefono = $('#nuevoTelefono').val();
	var direccion = $('#nuevaDireccion').val();
	var referencia = $('#nuevaReferencia').val();

	swal({

	  title: "¿Está seguro que desea realizar el pedido?",
	  text: "Una vez aceptado puede ir a pagar y recoger sus productos en la tienda",
	  type: "warning",
	  showCancelButton: true,
	  cancelButtonText: "Cancelar",
	  confirmButtonColor: "#47a60b",
	  ...
	  closeOnCancel: true
		...
		if (result.value) {
			...
			datosProducto.append("id_usuario", id_usuario);
			datosProducto.append("productos", listaCarrito);
			datosProducto.append("sumaCarrito", sumaCarrito);

			datosProducto.append('recibidor', nombre_reibidor);
			datosProducto.append('telefono', telefono);
			datosProducto.append('direccion', direccion);
			datosProducto.append('referencia', referencia);
			...
			$.ajax({
				...
				/*=============================================
				Eliminar todos los elementos que se encuentran en el localStorage
				=============================================*/
				...
				swal({
				  title: "El pedido se realizo con éxito",
				  text: "",
				  type: "success",
						...
						'<div clas="row itemCarrito">'+
							...
							'<div class="col-md-2 col-sm-3 col-xs-8">'+
								...
								'<div class="col-xs-8">'+
									...
									'<center>'+
										'<input type="number" class="form-control cantidadItem" min="1" max="'+item.stock+'" value="'+item.cantidad+'" precio="'+precio+'" idProducto="'+item.idProducto+'" item="'+index+'">'+	
										'<input id="stockProducto" type="hidden" value="'+item.stock+'">'+
									'</center>'+
...
$(document).on("change", ".cantidadItem", function(){
	...
	if ($(this).val() > parseInt($('#stockProducto').val())) {

		swal({
			title: "¡Solamente quedan " + $('#stockProducto').val() + " unidades!",
			text: "",
			type: "warning",
			showCancelButton: false,
			confirmButtonColor: "#DD6B55",
		...
		$(this).val($('#stockProducto').val());

	} else if ($(this).val() < 1) {

		swal({
			title: "¡Error al ingresar la cantidad!",
			text: "",
			type: "warning",
			showCancelButton: false,
			confirmButtonColor: "#DD6B55",
...
$("#btnCheckout").click(function(){
	...
	if(tipoArray.find(checkTipo) == "fisico"){
		...
		$.ajax({
			url:rutaOculta+"vistas/js/plugins/estados.json",
			type: "GET",
			cache: false,
			contentType: false,
			processData:false,
...
$("#cambiarDivisa").change(function(){
	...
	if($("#seleccionarPais").val() == ""){
		...
		$("#cambiarDivisa").after('<div class="alert alert-warning">No ha seleccionado el estado de envío</div>');
...
$(".btnPagar").click(function(){
	...
	if(tipo == "fisico" && $("#seleccionarPais").val() == ""){
		...
		$(".btnPagar").after('<div class="alert alert-warning">No ha seleccionado el estado de envío</div>');
