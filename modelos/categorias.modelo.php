...
class ModeloCategorias{
	...
	static public function mdlMostrarCategorias($item, $valor){
		if($item != null){
			$stmt = Conexion::conectar()->prepare("SELECT * FROM categorias WHERE $item = :$item LIMIT 15");
	...
	static public function mdlEditarCategoria($datos){
		...
		$stmt = Conexion::conectar()->prepare("UPDATE categorias SET categoria = :categoria, ruta = :ruta, estado = :estado, oferta = :oferta, precioOferta = :precioOferta, descuentoOferta = :descuentoOferta, imgOferta = :imgOferta, finOferta = :finOferta WHERE id = :id");
